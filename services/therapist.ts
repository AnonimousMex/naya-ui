...
export const THERAPIST_SERVICE = {
  ...
  async listAppointments(
    ...
    try {
      ...
      const { data } = await HTTP.get<
        TSingleDataResponse<TListAppointmentResponse>
      >(url, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async listPatients(
    token: string,
  ): Promise<TSingleDataResponse<TListPatientsResponse>> {
    try {
      const { data } = await HTTP.get<
        TSingleDataResponse<TListPatientsResponse>
